...
{
  uint32_t status = flash_verify_erase_need((uint16_t *) EFLASH_MAIN_BASE_ADDRESS, (uint16_t*)&settings, ((unsigned int)sizeof(settings_t)));
...
void settings_store_startup_line(uint8_t n, char *line)
{
  ...
  uint32_t status = flash_verify_erase_need((uint16_t *) addr, (uint16_t*)line, ((unsigned int)LINE_BUFFER_SIZE));
